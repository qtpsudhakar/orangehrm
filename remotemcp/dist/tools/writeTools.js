...
export const getWriteTools = () => [
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                const leaveRequestId = args['leaveRequestId'];
                const response = await apiPut(`/api/v2/leave/employees/leave-requests/${leaveRequestId}`, {
                    action: args['action'],
                    comment: args['comment'] ?? '',
                });
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                const timesheetId = args['timesheetId'];
                const response = await apiPut(`/api/v2/time/timesheets/${timesheetId}`, { action: args['action'] });
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                const response = await apiPut(`/api/v2/recruitment/candidates/${candidateId}/shortlist`, {
