...
async function resolveAuth(baseUrl, username, password) {
...
function invalidateSession(baseUrl, username) {
...
async function handleMcpRequest(req, res) {
    ...
    const baseUrl = getHeader(req, 'ORANGEHRM_BASE_URL') ||
        process.env['ORANGEHRM_BASE_URL'] ||
        '';
    const username = getHeader(req, 'ORANGEHRM_USERNAME') ||
        process.env['ORANGEHRM_USERNAME'] ||
        '';
    const password = getHeader(req, 'ORANGEHRM_PASSWORD') ||
        process.env['ORANGEHRM_PASSWORD'] ||
        '';
    if (!baseUrl || !username || !password) {
        res.writeHead(400, { 'Content-Type': 'application/json' }).end(JSON.stringify({
            error: 'Missing credentials. Supply ORANGEHRM_BASE_URL, ORANGEHRM_USERNAME, ' +
...
const httpServer = createServer(async (req, res) => {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, ORANGEHRM_BASE_URL, ORANGEHRM_USERNAME, ORANGEHRM_PASSWORD');
    ...
    if (req.url === '/mcp') {
        await handleMcpRequest(req, res).catch((err) => {
            if (!res.headersSent) {
                res.writeHead(500, { 'Content-Type': 'application/json' }).end(JSON.stringify({ error: 'Internal server error', detail: String(err) }));
            }
...
httpServer.listen(PORT, () => {
    console.error(`[OrangeHRM RemoteMCP] HTTP server ready — http://localhost:${PORT}/mcp`);
    console.error(`[OrangeHRM RemoteMCP] Health check  — http://localhost:${PORT}/health`);
