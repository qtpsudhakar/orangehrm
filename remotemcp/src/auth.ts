 ...
 */
export const authContext = new AsyncLocalStorage<OrangeHrmAuth>();
...
export class OrangeHrmAuth {
  ...
  async login(): Promise<void> {
    ...
    const tokenMatch = (loginPageResp.data as string).match(
      /:token="&quot;([^&]+)&quot;"/,
    );
    ...
    const loginResp = await axios.post(
      `${this.config.baseUrl}/auth/validate`,
      formData.toString(),
      {
        headers: {
    ...
    if (postCookies.length > 0) {
      ...
      for (const c of [...this.sessionCookies, ...postCookies]) {
        const eq = c.indexOf('=');
        if (eq !== -1) merged.set(c.substring(0, eq), c);
      }
    ...
    if (loginResp.status === 302) {
      ...
      throw new Error(
        'OrangeHRM login failed — check ORANGEHRM_USERNAME / ORANGEHRM_PASSWORD',
      );
    }

    // Fallback: if for some reason we got a 200 directly (no redirect),
    // treat a landing on /auth/login as failure.
    const responseUrl = (loginResp.request as {res?: {responseUrl?: string}})
      ?.res?.responseUrl ?? '';
   ...
   */
  async withAuth<T>(fn: (client: AxiosInstance) => Promise<T>): Promise<T> {
...
let _auth: OrangeHrmAuth | null = null;

export function getAuth(): OrangeHrmAuth {
